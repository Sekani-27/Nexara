...
def _poll_once():
    ...
    for p in positions:
        ...
        sl_risk = (
            abs(p.price_open - p.sl) * p.volume * 100_000 * 0.0001
            if p.sl
            else ACCOUNT_SIZE * 0.01
        )
        open_risk += sl_risk
        pos_list.append(
            {
    ...
    closed_pnl = sum(
        float(d.profit) for d in deals if d.entry == 1
    )  # 1 = deal out (close)
...
def serve_dashboard():
    ...
    if not os.path.exists(html_path):
        ...
        return JSONResponse(
            {"error": "genuvia_edge_full.html not found"}, status_code=404
        )
