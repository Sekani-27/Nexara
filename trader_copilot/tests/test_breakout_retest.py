...
def test_channel_bearish_detection():
    ...
    assert channel.bias == BiasType.BEARISH, \
        f"Expected BEARISH bias, got {channel.bias}"
    assert channel.ninety_pct_level == channel.high, \
        "90% rule level should be the channel HIGH for a bearish channel"

    print(f"PASS — Bearish channel detected | High: {channel.high:.5f} | "
...
def test_channel_bullish_detection():
    ...
    assert channel is not None, "Should detect a channel"
    assert channel.bias == BiasType.BULLISH, \
        f"Expected BULLISH bias, got {channel.bias}"
    assert channel.ninety_pct_level == channel.low, \
        "90% rule level should be the channel LOW for a bullish channel"

    print(f"PASS — Bullish channel detected | High: {channel.high:.5f} | "
          f"Low: {channel.low:.5f} | 90% level: {channel.ninety_pct_level:.5f}")
...
def test_corrective_wedge_in_bearish_channel():
    ...
    assert wedge.direction == BiasType.BULLISH, \
        f"Rising wedge direction should be BULLISH (correction), got {wedge.direction}"

    print(f"PASS — Corrective rising wedge in bearish channel detected | "
...
def test_wedge_rejected_when_range_expanding():
    ...
    assert wedge is None, \
        "Expanding range should NOT be detected as a corrective wedge"
...
def test_breakout_body_rule_bearish():
    ...
    assert body_break.close   < wedge_low, "Body-close candle should close below wedge low"
    ...
    assert result[1] == 11,     f"Breakout should be at index 11 (body_break), got {result[1]}"
...
def test_ob_retest_detected():
    ...
    assert retest_idx == 5, f"Retest should be at index 5 (retest candle), got {retest_idx}"

    print(f"PASS — OB retest detected at candle index {retest_idx} | OB top: {ob.top:.5f}")
...
def test_ninety_pct_rule_invalidation():
    ...
    candles = [
        ...
        make_candle(1.09600, 1.09750, 1.09580, 1.09720, i=13),   # Closes above 90% level!
...
def test_full_pipeline_bearish():
    ...
    for i in range(WEDGE_CANDLES):
        ...
        l = wedge_base + tiny_drift - spread * 0.05
        c = wedge_base + tiny_drift + spread * 0.6
        candles.append(make_candle(o, h, l, c, i=CHANNEL_CANDLES + i))
    ...
    candles.append(make_candle(ob_open, ob_high, ob_low, ob_close,
                               i=CHANNEL_CANDLES + WEDGE_CANDLES))
    ...
    candles.append(make_candle(bo_open, bo_high, bo_low, bo_close,
                               i=CHANNEL_CANDLES + WEDGE_CANDLES + 1))
    ...
    for i in range(RETEST_CANDLES):
        ...
        l = bo_close + i * 0.00003 - 0.00001
        c = bo_close + i * 0.00003 + 0.00002
        candles.append(make_candle(o, h, l, c,
                                   i=CHANNEL_CANDLES + WEDGE_CANDLES + 2 + i))
    ...
    print(f"PASS — Single engine.analyse() call: breakout_scan fix confirmed ✓")
    print(f"       Pattern  : {result.pattern_name}")
    print(f"       Channel  : {result.channel_bias.value} | 90%: {result.ninety_pct_level:.5f}")
    print(f"       Wedge    : {result.wedge_low:.5f}–{result.wedge_high:.5f}")
    print(f"       OB       : {result.order_block.bottom:.5f}–{result.order_block.top:.5f}")
...
def test_no_signal_without_breakout():
    ...
    for i in range(20):
        ...
        candles.append(make_candle(o, h, l, c, i=80 + i))
    ...
    if result is None:
        ...
        assert result.valid
        print(f"PASS — Engine found a setup (breakout confirmed): {result.pattern_name}")
